	...
	public class UpdateMultiPassStereo : MonoBehaviour
	{
		...
		private void LateUpdate()
		{
			...
			if (_foundCamera == null)
			{
				...
				if (_foundCamera == null)
				{
					Debug.LogWarning("[AVPro Video] Cannot find main camera for UpdateMultiPassStereo, this can lead to eyes flickering");
					if (Camera.allCameras.Length > 0)
					{
						_foundCamera = Camera.allCameras[0];
						Debug.LogWarning("[AVPro Video] UpdateMultiPassStereo using camera " + _foundCamera.name);
					}
				}
			}
